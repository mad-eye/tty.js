...
function Server(conf) {
  ...
  this.on('listening', function() {
    //this isn't true if a port is passed in
    //self.log('Listening on port \x1b[1m%s\x1b[m.', self.conf.port);
  });

  this.init();
  this.createPty(process.stdout.columns, process.stdout.rows, function(error){
    if (error){
      console.error(error);
    }
  });
...
Server.prototype.createPty = function(cols, rows, callback){
  var self = this;

  var shell = typeof this.conf.shell === 'function'
    ? this.conf.shell(this)
    : this.conf.shell;

  var shellArgs = typeof this.conf.shellArgs === 'function'
  ...
  term = pty.fork(shell, shellArgs, {
    ...
    cols: cols,
    rows: rows,
    cwd: this.conf.cwd || process.env.HOME
  });
  ...
  if (this.readonly){
    ...
    //HACK after the shell has loaded run this command before
    //listening to shell data, wait half a second then start
    //listening, write a newline to trigger the first prompt :-)

    //maybe it's better to let the user see the export statement

    term.write("export PS1=\"" + this.conf.prompt + "\"\n");
    ...
    term.on("close", function(){
      console.log("Exiting MadEye. Restart your session with `madeye --term`");
      process.exit(0);
    })
  }
  else {
    term.on('close', function(data){
      self.createPty(process.stdout.columns, process.stdout.rows, function(err){
        console.error("Error on terminal recreation ", err);
      });
    });
  }
};
...
Session.prototype.handleKill = function(id) {
  ...
  terms[id].destroy();
  delete terms[id];
  //TODO automatically recreate a new terminal here
};
...
Session.prototype.handleDisconnect = function() {
  ...
  function destroy() {
    ...
    while (i--) {
      term = terms[key[i]];
      delete terms[key[i]];
      //TODO destroy term if this is the last session connected
    }
